...
def test_generates_top_n_offers():
    offers = PersonalOfferEngine(top_n=3).generate_offers(SegmentType.NEW_USER, price_sensitivity=0.7)
    assert len(offers) <= 3
    assert all(0 <= o.score <= 1 for o in offers)


def test_offers_sorted_by_score_descending():
    offers = PersonalOfferEngine(top_n=5).generate_offers(SegmentType.PRICE_SENSITIVE, price_sensitivity=1.5)
...
def test_unknown_segment_returns_empty_list():
    offers = PersonalOfferEngine().generate_offers(SegmentType.NEW_USER, price_sensitivity=0.7)
