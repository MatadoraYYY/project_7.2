...
def _create_customer(client, external_ref="anon-offer-001"):
    return client.post("/api/v1/customers", json={"external_ref": external_ref}).json()["id"]
...
def test_generate_offers_for_missing_customer_returns_404(client):
    assert client.get("/api/v1/offers/generate/99999").status_code == 404
...
def test_apply_nonexistent_offer_returns_404(client):
    assert client.post("/api/v1/offers/apply/does-not-exist", json={"converted": True}).status_code == 404
...
def test_offer_analytics_reflects_generated_offers(client):
    ...
    assert response.status_code == 200
    assert response.json()["total_offers_generated"] > 0
