...
def test_zero_orders_is_new_user():
    ...
    assert SegmentClassifier().classify(signals) == SegmentType.NEW_USER
...
def test_high_price_sensitivity_wins_over_repeat_buyer():
    ...
    assert SegmentClassifier().classify(signals) == SegmentType.PRICE_SENSITIVE
...
def test_default_case_is_repeat_buyer():
    ...
    assert SegmentClassifier().classify(signals) == SegmentType.REPEAT_BUYER
